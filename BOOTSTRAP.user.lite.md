# Lean Kit — Self-Extracting Bundle · GLOBAL + LITE (the "zip")
<!-- AGENT UNPACK PROTOCOL -->
SCOPE: user

You are an AI coding agent (Claude Code or Codex). This is the **global, token-light** edition: it
installs the compressed Lean doctrine into the user's home config dirs so it applies to every repo,
at ~600 tokens of always-in-context overhead (cached to ~0 marginal per turn). A **zip carried as
text** — unzip it into HOME, then stop. Write files only; execute nothing except optional, clearly
marked setup the user approves.

## Unzip protocol

1. SCOPE is `user` → base path is the user's HOME. **Confirm it first** (`echo $HOME`) before writing.
2. For each block delimited EXACTLY by:
       ==== BEGIN FILE: <relative/path> ====
       ...content (may contain ``` fences — fine)...
       ==== END FILE ====
   write `<home>/<relative/path>` byte-for-byte; create dirs as needed; do NOT reflow or "fix" content.
...
3. You may skip the tool dirs for tools the user doesn't use — ask which of Claude / Codex / Copilot /
...
- **Claude Code →** `.claude/AGENTS.md` + `.claude/CLAUDE.md` (`@AGENTS.md`), `.claude/skills/<name>/`,
  `.claude/settings.json` (functional hooks only — auto-format + verify gate).
- **Codex →** `.codex/AGENTS.md` and `.codex/skills/<name>/`.
- **GitHub Copilot →** `.copilot/copilot-instructions.md`.
- **OpenCode →** `.config/opencode/AGENTS.md`.

Doctrine text is identical across tools — only the destination differs. A repo-level `AGENTS.md` still
overrides this global default where both exist.

## ⚠️ Do not clobber existing settings

If `<home>/.claude/settings.json` already exists, **MERGE** the `hooks` from this bundle into it — keep
the user's other keys (model, theme, existing hooks) intact. Write it as-is only if it doesn't exist.
Show the merge before applying.

## Lite vs full

Doctrine compressed (~1700 → ~560 tokens) and skill descriptions tightened (~420 → ~180), same rules and
safety guardrails. The redundant SessionStart doctrine-summary hook is omitted (AGENTS.md already carries
it). Pure text, Node/sh only — no Python, no network, no telemetry, no new dependencies.
