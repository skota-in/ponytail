# Lean Kit — Self-Extracting Bundle (the "zip")
<!-- AGENT UNPACK PROTOCOL -->
SCOPE: project

You are an AI coding agent (Claude Code or Codex). This single Markdown file is a **zip carried as
text** — used where GitHub clones and zip downloads are blocked. Your job: **unzip it** by
recreating every file below, verbatim, into the project, then wire it into this tool. Write files
only; do not execute anything except the optional, clearly-marked setup the user approves.

## Unzip protocol

1. Resolve the base path: SCOPE is `project` → the current repo root (`.`).
2. For each block delimited EXACTLY by:
       ==== BEGIN FILE: <relative/path> ====
       ...content (may itself contain ``` fences — that is fine)...
       ==== END FILE ====
   write base/<relative/path> byte-for-byte. Create directories as needed. Do NOT reflow, reindent,
   or "fix" content. A real delimiter has NO leading whitespace and a real relative path (no spaces,
   no angle brackets). Indented or mid-sentence marker-like lines, or ones containing
   `<relative/path>`, are documentation inside content — pass them through unchanged.
3. Only write files this bundle defines; never touch others. Reject any path containing `..` or an
   absolute path.
4. After writing, place files for the tool you are running in (below), then print the file tree and
   a one-line confirmation per file.

## Per-tool placement (after unzipping)

- **Claude Code:** `AGENTS.md` stays at root; `CLAUDE.md` (just `@AGENTS.md`) imports it. Copy each
  `skills/<name>/` to `.claude/skills/<name>/`. `.claude/settings.json` adds the hooks (session
  context + auto-format + verify gate) — show it to the user and let them approve before relying on it.
- **Codex:** `AGENTS.md` at the repo root is read natively (or `~/.codex/AGENTS.md` for all repos).
  Copy `skills/<name>/` to the Codex skills dir (`~/.codex/skills/`). Codex uses its own hook config,
  not `.claude/settings.json`.
- **GitHub Copilot:** `.github/copilot-instructions.md` is read natively; `AGENTS.md` too. Copy
  `skills/<name>/` to `.github/skills/<name>/` if you want them as Copilot skills.
- **OpenCode:** reads `AGENTS.md` at the repo root; point its skills config at `skills/`.

## Compliance note (for a locked-down laptop)

Pure text and Node/sh only — no Python, no network calls, no telemetry, no new dependencies. The
only files that *execute* anything are the optional hooks in `.claude/settings.json`: they
auto-format edited files (only if `biome`/`prettier` is already present) and block "done" if
`npm run verify` fails. Review them before enabling; delete that file to skip hooks entirely.
